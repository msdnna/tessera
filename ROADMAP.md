...
Текущие версии — `make version`. На 2026-06-22: backend `0.52.0` ·
frontend `0.87.2` · android `0.31.6`. Следующая миграция — `0032`.
...
**СЛЕДУЮЩИЙ КРУПНЫЙ ШАГ — Item 5: масштаб меняет ГРАНУЛЯРНОСТЬ оси (отдельный thread):**
Зум должен менять не только плотность дней, но и единицу оси: при сильном отдалении группировать
по **неделям** (числа недель/диапазоны), при сильном приближении — добавлять к дням **часы**. Тиры по
`dayW`: weeks ↔ days ↔ days+hours; плавный переход. **Оба клиента.** На Android фундамент уже есть —
`TimelineAxisCanvas` рисует ось, тиры = «что рисуем при данном `dayW`»; на web — `TimelineDayRow`/
`monthBands` + `ZOOM`-массив. Главные вопросы дизайна: пороги тиров, формат меток недель/часов, как
бары/гридлайны/ghost ложатся на под-дневную/над-дневную сетку. Начать в новом треде.

**Прочий бэклог видов:**
- **Виртуализация** длинных списков в таймлайне/матрице на **web** (Android сделан); web pan/hover уже есть.
