...
### 6. Управление пользователями — 🔜 СЛЕДУЮЩЕЕ (план одобрен в принципе 2026-06-15)
- Порядок: **U1** права+профиль → **U2** инвайты+деактивация → **U3** админ-панель. Критбаги
  расчищены. Текущее состояние тонкое: `is_admin` не используется, роли плоские, добавление
  только существующих юзеров. Перед U1/U2 подтвердить SMTP (инвайты по email) и хранение
  аватаров. Детали — auto-memory `project-user-management-next`.
