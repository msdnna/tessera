...
Текущие версии — `make version`. На 2026-06-19: backend `0.47.0` ·
frontend `0.78.2` · android `0.21.2`. Следующая миграция — `0028`.
...
## 14. Новые представления доски — Гант / Таймлайн / Матрица Эйзенхауэра — 🔜 В РАБОТЕ (по одному, в полном функционале)
...
рисуют *отрезок* (start→end) → нужен **`start_date`** (миграция **0028**, nullable timestamptz).
Матрице Эйзенхауэра и существующим видам start не нужен.

**Рекомендованный порядок (по возрастанию объёма; каждый шаг готовит почву следующему):**

1. **Матрица Эйзенхауэра** — ✅ СДЕЛАНО (backend 0.48.0 / web 0.79.0 / android 0.22.0, 2026-06-19).
   2×2 (Важно×Срочно), переиспользует `TaskCard`. Квадрант *выводится* из полей — ось **Важно** =
   priority≥high, ось **Срочно** = `due_date` в пределах окна (дефолт 7 дней / просрочено). **derive
   + ручной override** (полный a reference tracker-вариант): nullable `eisenhower_quadrant` (миграция **0028**),
   `PATCH /tasks/:id/eisenhower {quadrant:0-3|null}` (null = «вернуть на авто»). Карточка сохраняет
   priority-бейдж. **Web** — drag между квадрантами (vuedraggable) + «вручную»-пин для сброса +
   per-quadrant «＋». **Android** — нет cross-container drag в Compose → override через per-card меню
   («Переместить в…» + «Вернуть на авто») + per-quadrant «＋». Иконка `apps` (2×2). Бэклог матрицы:
   web-виртуализация квадрантов при больших списках; Android drag (если понадобится).
2. **Таймлайн** — вводит `start_date` + движок горизонтальной шкалы времени с отрезками-барами.
   Референс a reference tracker Timeline: свимлейны (группировка по area/исполнителю/тегу), бар каждой задачи
   start→due, вертикальная «сегодня», «Unscheduled tasks» на лейн, сайдбар-счётчики (overdue / no
   effort / unscheduled). Drag-move + resize краёв меняют start/due. **Без связей между задачами.**
3. **Гант** — переиспользует таймлайн-движок (та же ось/бары/drag), добавляет: **зависимости задач**
   (новая таблица `task_dependencies` blocks/blocked_by + рисование стрелок + drag для создания связи) и
   левую панель-список задач (строка = задача). Самый тяжёлый — граф зависимостей это отдельный
   под-проект. По референсу a reference tracker Gantt (стрелки, drag-resize, today-линия).
...
- **Оценка задач (estimation)** — без неё трекер неполон; питает «effort»-индикаторы таймлайна/Ганта.
  **Концепция (согласовано 2026-06-19):** хранить оценку в каноне = **минуты** (nullable, int), а
  ввод/вывод конвертировать через настройку единиц. **Настройка двухуровневая** (как с префиксами
  тегов): **per-workspace** — общий паттерн-дефолт для всех проектов, **per-project** — частный
  override (напр. проект «Дом» со своими единицами). Провайдеро-нейтрально.
  - **Время:** канон-минуты + конфиг **рабочего дня/недели** `hours_per_day` (дефолт 8) /
    `days_per_week` (дефолт 5) — **тоже двухуровневый** (workspace-дефолт + project-override); ввод
    «3d 4h»/«1w»/«90m» нормализуется в минуты, вывод форматируется по рабочему дню/неделе (так «3 дня»
    = 24 рабочих часа, не 72).
  - **Стори-поинты:** канон = число по шкале (Фибоначчи 1/2/3/5/8/13 или T-shirt S/M/L→числа), без
    конвертации во время.
  - **Кастом:** именованная единица (метка + число), опц. шкала.
  - Роллап: оценка родителя = сумма подзадач (или вручную). Поверхности: поле в TaskModal/DueEditor,
    чип на карточке («⏱30h»), агрегаты в лейн-хедерах таймлайна/Ганта. **Оценка ≠ трекинг времени**
    (учёт затраченного — отдельный, более поздний кандидат). Открыто: нужны ли смешанные единицы на
    одной доске (если проекты доски с разными единицами).
