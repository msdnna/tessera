...
Текущие версии — `make version`. На 2026-06-20: backend `0.52.0` ·
frontend `0.85.0` · android `0.29.1`. Следующая миграция — `0032`.
...
**Открытый бэклог по видам (таймлайн/гант) — кандидаты на текущий заход:**
- **Android-паритет с web:** группировка по исполнителю / без группировки; pan (панорама); hover/preview
  карточка; per-layout состояние тулбара (Android всё ещё держит один общий конфиг).
- **Отзывчивость зума** (web wheel + Android pinch) — «зум ощущается тормозным», помечено как fix-later.
- **Виртуализация** длинных списков в таймлайне/матрице (web) при большом числе строк.
