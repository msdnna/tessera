...
android-паритет суб-баров — в бэклоге). Запланированные подзадачи (со `start`/`due`) рисуются тонкими
суб-барами под родительским баром в той же строке (`rowHeight`/`SUB_STEP`/`findTask`-save; на Ганте без
узелка-связи). Нейтральная пунктирная курсор-линия с приколотой к верху пилюлей даты (в тире «часы» — со
...
- **GitLab журнал синхронизации — ✅ СДЕЛАНО** (2026-06-23, backend 0.54.0 / web 0.93.0, миграция 0033).
  ...
  Открытый хвост: визуальный прогон в живом UI (headless для NModal ненадёжен) и Android-паритет.
