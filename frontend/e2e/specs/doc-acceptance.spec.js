...
test('документы: импорт → согласование → откат на одном документе', async ({
  ...
  try {
    ...
    // Обсуждение закрывается не для удобства: панели не исключают друг друга, а
    // ширины у них фиксированные, поэтому с тремя открытыми (обсуждение +
    // история + связи) полоса текста на экране 1280px схлопывается до 9 px и
    // документ становится нередактируемым. Это отдельный дефект (#2738); здесь
    // спека его обходит, а не маскирует.
    await page.locator('button[title="Скрыть обсуждение"]').click()
