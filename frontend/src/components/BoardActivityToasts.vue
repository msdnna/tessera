...
.activity-stack {
  ...
  left: 16px;
  bottom: 16px;
  /* НИЖЕ полосы оверлеев Naive (она начинается с 2000: vdirs/z-index-manager
     раздаёт z-index счётчиком от 2000 на каждый монтируемый поповер/дропдаун/
     модалку). Это не «чтобы не мешало», а лечение мерцания, см. ниже.

     Пока стопка была ВЫШЕ маски модалки (пробовали 2200 и 8000), она оставалась
     единственным, что рисуется поверх маски. Маска анимирует opacity, то есть на
     время анимации становится отдельным слоем композитора, а всё, что рисуется
     ПОВЕРХ такого слоя, Chromium обязан вынести в свой слой (promotion by
     overlap). Новый слой презентуется до того, как растеризован, и его тайлы
     заливаются фоновым цветом элемента — ровно один белый кадр без содержимого.

     Замер по видео из задачи (687 кадров, 30 fps): ровно 4 битых кадра, все —
     первый кадр закрытия модалки (4 из 4 закрытий), область тоста залита ровным
     153 = 255 × 0.6, то есть белый фон карточки под 40-процентной маской, без
     текста, аватара и кнопок. Остальной экран в этот кадр совпадает с предыдущим
     побайтово: доска не мерцает, потому что она под маской, а не над ней.

     Отсюда правило: стопка не должна рисоваться поверх анимируемой маски. Цена —
     при открытой модалке тост притушен маской и не кликабелен. Выше блокирующих
     оверлеев приложения (лоадер 9000, оверлей связи 9001-9002) не поднимать. */
  z-index: 1900;
  display: flex;
  ...
  /* Один постоянный слой композитора на ВСЮ стопку, а не по слою на тост.
     ...
     layer и пересобирает его на каждую смену состава. На видео после этой правки
     мерцания на входе/уходе самих тостов и на перетаскивании карточек уже нет —
     оставалось только описанное выше, из-за маски. */
