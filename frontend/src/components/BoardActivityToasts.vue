...
.activity-stack {
  ...
  left: 16px;
  bottom: 16px;
  /* ВЫШЕ динамической полосы оверлеев Naive и ниже блокирующих оверлеев
     приложения (лоадер 9000, оверлей связи 9001-9002).

     Нельзя брать значение из 2000-4500: vdirs/z-index-manager раздаёт z-index
     счётчиком от 2000, инкрементя его на каждый монтируемый поповер/дропдаун/
     модалку, так что исходные «2200, вроде выше модалки» переставали быть правдой
     после сотни оверлеев за сессию.

     История (#2695): какое-то время здесь стояло 1900 — я считал, что белый кадр
     при закрытии модалки даёт promotion by overlap над анимируемой маской. Это
     оказалось неверно: тост тогда лежал внутри слота <n-spin> в KanbanBoard, и
     слой ему создавала анимация opacity у .n-spin-content на фоновой перезагрузке
     доски (см. `load()` там же). Это же объясняет замер по видео — залитая
     ровным цветом область БЕЗ содержимого: свежепромотированный слой уходит на
     экран до растеризации. Простое притушение маской content не стёрло бы.
     Совпадение по времени тоже сходится: битые кадры были на 4 закрытиях модалки
     из 4 (закрытие с правкой → onChanged → перезагрузка) и ни на одном открытии.

     Теперь стопка вынесена из <n-spin>, поэтому опускать её под маску незачем —
     возвращаем тост поверх модалки, кликабельным. */
  z-index: 8000;
  display: flex;
