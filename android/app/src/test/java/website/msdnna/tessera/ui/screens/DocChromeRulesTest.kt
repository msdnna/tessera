...
class DocChromeRulesTest {
    ...
    fun `an open document stops the sidebar swipe, an open drawer restores it`() {
        assertThat(drawerGesturesEnabled(timelineLike = false, documentOpen = false, drawerOpen = false)).isTrue()
        // The regression: scrolling a document pulled the sidebar in.
        assertThat(drawerGesturesEnabled(timelineLike = false, documentOpen = true, drawerOpen = false)).isFalse()
        assertThat(drawerGesturesEnabled(timelineLike = true, documentOpen = false, drawerOpen = false)).isFalse()
        // Open, the drawer must stay draggable — that is how it closes.
        assertThat(drawerGesturesEnabled(timelineLike = false, documentOpen = true, drawerOpen = true)).isTrue()
        assertThat(drawerGesturesEnabled(timelineLike = true, documentOpen = true, drawerOpen = true)).isTrue()
    }
}
